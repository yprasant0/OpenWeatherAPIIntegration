namespace :air_quality do
  ...
  task :fetch_history, [:months_back, :days_from_start] => :environment do |t, args|
    months_back = args[:months_back].to_i || 12 # Default to 12 months if not specified
    days_from_start = args[:days_from_start].to_i || 10 # Default to 10 days if not specified
